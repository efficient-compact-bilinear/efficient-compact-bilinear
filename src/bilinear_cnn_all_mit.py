...
class BCNN(torch.nn.Module):
    def __init__(self):
        ...
        self.fc = torch.nn.Linear((512//self.d1*self.d2)**2, 67)
    ...
    def forward(self, X):
        ...
        Xfold2 = X.view(-1,self.d1,512//self.d1).permute(0,2,1)
...
class BCNNManager(object):
    ...
    def __init__(self, options, path):
        ...
        self._solver = torch.optim.SGD(
            [self._net.module.w1, self._net.module.w2, self._net.module.w3, self._net.module.w4] +  list(self._net.module.fc.parameters()) + list(self._net.module.features.parameters()), lr=self._options['base_lr'],
            momentum=0.9, weight_decay=self._options['weight_decay'])
        self._scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self._solver, mode='max', factor=0.1, patience=7, verbose=True,
            threshold=1e-4)
...
def main():
    ...
    path = {
        ...
        'model': os.path.join(project_root, 'model_mit', args.model),
    }
    for d in path:
        if d == 'model':
